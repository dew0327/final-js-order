...
public class PolicyHandler{
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void wheneverPointSended_PointInfoUpdate(@Payload PointSended pointSended){

        if(pointSended.isMe()){
            System.out.println("##### listener PointInfoUpdate : " + pointSended.toJson());
            Optional<Order> orderOptional = orderRepository.findById(pointSended.getOrderId());
            Order order = orderOptional.get();
            if("Point : Point SENDED".equals(pointSended.getStatus())){
                order.setPointStatus("ORDER : point SENDED SUCCESS");
            }
            orderRepository.save(order);
        }
    }
